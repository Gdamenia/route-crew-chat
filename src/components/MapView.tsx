...
export function MapView({ onDriverSelect }: MapViewProps) {
  const { myLocation, nearbyDrivers, setNearbyDrivers, upsertNearbyDriver, removeNearbyDriver, geoError, setGeoError, locationSyncFailed } = usePresenceStore();
  ...
  useEffect(() => {
    ...
    const myIconHtml = `
      <div style="position:relative;width:20px;height:20px;">
        <div style="position:absolute;inset:0;border-radius:50%;background:#E8A020;opacity:0.3;animation:pulse 2s infinite;"></div>
        <div style="position:absolute;inset:4px;border-radius:50%;background:#E8A020;border:2px solid #fff;"></div>
      </div>
    `;
  ...
  const retryLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      () => { /* watchPosition will handle it */ },
      () => setGeoError('Still unable to get location. Check browser settings.'),
  ...
  return (
    <div className="relative w-full h-full min-h-[400px]">
      ...
      {geoError && (
        <div className="absolute top-2 left-2 right-2 z-20 bg-amber-600 text-white rounded-xl px-4 py-3 flex items-center justify-between gap-2 shadow-lg">
          <p className="text-sm font-medium flex-1">{geoError}</p>
          <button onClick={retryLocation} className="bg-white/20 hover:bg-white/30 text-white text-xs font-bold px-3 py-1.5 rounded-lg flex-shrink-0">
            Retry
          </button>
        </div>
      )}

      {locationSyncFailed && !geoError && (
        <div className="absolute top-2 left-2 right-2 z-20 bg-amber-800/90 text-amber-200 rounded-xl px-4 py-2 text-xs font-medium shadow-lg">
          Your location isn't visible to others — check your profile.
        </div>
      )}
