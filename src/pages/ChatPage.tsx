...
export default function ChatPage() {
  ...
  const channelName = (location.state as any)?.channelName ?? 'Route Channel';
  ...
  useEffect(() => {
    if (!channelId) return;
    channelService.getMessages(channelId).then((data) => {
      setMessages(channelId, data);
      setLoading(false);
      setTimeout(() => bottomRef.current?.scrollIntoView(), 100);
    });
    return channelService.subscribeToMessages(channelId, (msg) => {
  ...
  const handleSend = async () => {
    if (!text.trim() || !profile || !channelId || sending) return;
    const trimmed = text.trim();
    setText('');
    setSending(true);
    try {
      await channelService.sendMessage(channelId, profile.user_id, trimmed);
    } catch {
      setText(trimmed);
    } finally {
      setSending(false);
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex-shrink-0 flex items-center gap-3 px-4 py-3 bg-card border-b border-border">
        ...
        <button
          onClick={() => navigate(`/voice/${channelId}`, { state: { channelName } })}
